...
namespace MaxFlow {
    ...
    void add_edge(int u, int v, ll f) {
        G[u].push_back({v, (int) G[v].size(), f, ++idx});
...
int main() {
    ...
    MaxFlow::solve(ss, tt);
    if (MaxFlow::flow != sum) { //不是满流
        printf("NO\n");
        return 0;
    }
    printf("YES\n");
    for (int i = 1; i <= n; i++) {
        for (auto o : MaxFlow::G[i]) {
            if (o.id == 0) continue;
            ans[o.id] += MaxFlow::G[o.v][o.rev].f; //加上新图的实际流量
        }
    }
    for (int i = 1; i <= m; i++) {
        printf("%lld\n", ans[i]);
